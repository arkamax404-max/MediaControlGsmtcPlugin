...
def inspect_sdk():
    ...
    for tile in MINIMAL_PNG_URIS:
        try:
            decoded = base64.b64decode(tile.split(",", 1)[1], validate=True)
            with Image.open(BytesIO(decoded)) as image:
                image.verify()
                if image.format != "PNG" or image.size != (1, 1):
                    raise RuntimeError("Synthetic mosaic fixture is not a minimal PNG")
        except (OSError, ValueError, TypeError) as exc:
            raise RuntimeError("Synthetic mosaic fixture is not a decodable PNG") from exc
    ...
    for index, action in enumerate(MOSAIC_ACTIONS):
        ...
        settings = ({"secondaryAction": "toggle",
                     "audioTarget": "process:spotify.exe", "badgeColor": "#123456"}
                    if index == 0 else {})
    ...
    for index, context_value in enumerate(mosaic_contexts):
        ...
        if index == 0:
            try:
                mosaic_svg = base64.b64decode(
                    data.split(",", 1)[1], validate=True).decode("utf-8")
            ...
            required = (
                ...
                '<path fill="#FFFFFF" d="M29 24h15v52H29zm27 0h15v52H56z"/>',
            )
            if not isinstance(data, str) or not data.startswith("data:image/svg+xml;base64,") \
        ...
        if action == MUTE_TOGGLE_UUID:
            if ([(item.get("type"), item.get("path"), item.get("data"), item.get("textData"),
                ...
                raise RuntimeError(f"Unexpected integrated mute-toggle payloads: {items}")
                 ...
                 item.get("textData"), item.get("showtext"), item.get("key"),
                 item.get("actionid"))
                for item in items] != [
    ...
    for index, action in enumerate(TRANSPORT_DISPLAY):
        ...
        if action == TOGGLE_UUID:
            if ([(item.get("type"), item.get("path"), item.get("data"),
                    item.get("textData"), item.get("showtext"), item.get("key"),
                    item.get("actionid"))
                   for item in items] != [
                    (1, None, transport_icon_data_uri(action, True),
                     "Pause", True, key, actionid),
                    (1, None, transport_icon_data_uri(action),
                     "Play", True, key, actionid)]):
