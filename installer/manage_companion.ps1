...
function Test-ExactRuntime([int]$ProcessId, [string]$Path) {
    if ($DryRun) { $runtime = $script:runtime; return $runtime.process_alive -and $runtime.pid -eq $ProcessId -and $runtime.path -ceq $Path -and $runtime.listener -and $runtime.mutex -and $runtime.health -and $runtime.companion_version -eq $script:companionVersion }
    try { $process = Get-Process -Id $ProcessId -ErrorAction Stop; if ($process.HasExited -or [IO.Path]::GetFullPath($process.Path) -cne $Path) { return $false }; $listeners = @(Get-NetTCPConnection -LocalAddress '127.0.0.1' -LocalPort 43821 -State Listen -ErrorAction Stop); if ($listeners.Count -ne 1 -or $listeners[0].OwningProcess -ne $ProcessId -or -not (Test-Mutex)) { return $false }; $health = Invoke-RestMethod -Uri 'http://127.0.0.1:43821/health' -TimeoutSec 1; $instance = [guid]::Empty; return $health.service -eq 'd200-gsmtc-bridge' -and $health.companion_version -eq $script:companionVersion -and $health.api_major -eq 1 -and $health.api_minor -ge 0 -and $health.status -in @('ready','degraded') -and [guid]::TryParse([string]$health.instance_id,[ref]$instance) } catch { return $false }
}
...
function Start-Prior {
    if ($DryRun) { if (Test-Failure 'Restart') { throw 'Prior restart failed' }; if (Test-Failure 'PriorImmediateExit') { $script:memoryTask.Status='Ready'; $script:runtime.process_alive=$false; throw 'Prior process exited' }; $script:memoryTask.Status='Running'; $script:owner='Prior'; $script:runtime=[ordered]@{process_alive=$true;listener=$true;mutex=$true;health=$true;companion_version=$script:companionVersion;pid=101;path=$script:memoryTask.Target;stable_polls=0;stop_invoked=$false}; foreach($poll in 1..6){if(Test-Failure "PriorPoll$poll"){$script:runtime.health=$false};if(-not (Test-ExactRuntime 101 $script:memoryTask.Target)){throw 'Prior runtime invalid'};$script:runtime.stable_polls=$poll}; return }
...
function Start-Candidate {
    if ($DryRun) { $script:candidateStarted=$true; $script:candidatePid=202; $script:candidatePath=$exe; $script:owner='Candidate'; $script:runtime=[ordered]@{process_alive=$true;listener=$true;mutex=$true;health=(!(Test-Failure 'Health'));companion_version=$script:companionVersion;pid=202;path=$exe;stable_polls=1;stop_invoked=$false}; if (Test-Failure 'CandidateAliveNoListener') { $script:runtime.listener=$false; $script:runtime.mutex=$false; $script:runtime.health=$false }; return }
...
try {
    ...
    $expectedStatusPath = Assert-Canonical (Join-Path $LocalAppDataRoot 'Programs\GSMTCD200Controller\installer\activation-status.txt')
    if (-not $DryRun) {
        if (-not $StatusPath) { throw 'Unexpected status path' }
        $candidateStatusPath = Assert-Canonical $StatusPath
        if ($candidateStatusPath -ine $expectedStatusPath) { throw 'Unexpected status path' }
        $script:statusPathIsSafe = $true
    }
    $VersionRoot = Assert-Canonical $VersionRoot; $DataRoot = Assert-Canonical $DataRoot
    $versionPattern = '^' + [regex]::Escape((Join-Path $LocalAppDataRoot 'Programs\GSMTCD200Controller\versions\')) + '((?:0|[1-9]\d*)\.(?:0|[1-9]\d*)\.(?:0|[1-9]\d*))\\bridge$'
    if ($VersionRoot -cnotmatch $versionPattern) { throw 'Unexpected companion version root' }
    $script:companionVersion = $Matches[1]
    if ($DataRoot -cne (Join-Path $LocalAppDataRoot 'GSMTCD200Controller') -or $TaskName -cne 'GSMTCD200Controller-Companion') { throw 'Unexpected companion path or task' }
} catch {
    Save-Phase
